...
if [ -z "wont-run" ]; then
  ...
  cd $SCRATCHDIR
  tar -xf rose_videos.tar --directory . --checkpoint=.10000
  # mv $SCRATCHDIR/vids/RoseYoutu-full $SCRATCHDIR
  python scripts/extract_rose_youtu_videos.py -i $SCRATCHDIR/RoseYoutu-full -o $SCRATCHDIR/rose_youtu_imgs100 -f 50 -b 256 && cd $SCRATCHDIR && tar cf rt_single.tar --checkpoint=.10000 rose_youtu_imgs && cp ryi7.tar /storage/brno2/home/petrmiculek/facepad/

  python scripts/extract_rose_youtu_videos.py -i $SCRATCHDIR/RoseYoutu-full -o $SCRATCHDIR/rose_youtu_imgs -f 2 -b 16 && tar cf ryi7.tar --checkpoint=.10000 rose_youtu_imgs  #  && cp ryi7.tar /storage/brno2/home/petrmiculek/facepad/
